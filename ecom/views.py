...
def generate_signature(total_amount, transaction_uuid, product_code, secret_key):
    input_string = f"total_amount={total_amount},transaction_uuid={transaction_uuid},product_code={product_code}"
    secret_key_bytes = secret_key.encode()
    input_string_bytes = input_string.encode()
    hmac_signature = hmac.new(secret_key_bytes, input_string_bytes, hashlib.sha256)
...
def customer_address_view(request):
    ...
    if 'product_ids' in request.COOKIES:
        ...
        if product_ids:
            ...
            products_in_cart = models.Product.objects.filter(id__in=product_ids_list)
    ...
    # Generate signature
    secret_key = '#vw(03o=(9kbvg!&2d5i!2$_58x@_-3l4wujpow6(ym37jxnza'  # Replace with your actual secret key provided by eSewa
    signature = generate_signature(total_amount, transaction_uuid, 'EPAYTEST', secret_key)
    ...
    return render(request, 'ecom/customer_address.html', context=context)
